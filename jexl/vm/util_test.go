...
func TestIntDivide(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := IntDivide(tt.a, tt.b)
			if got != tt.want {
				t.Fatalf("IntDivide(%v, %v) = %v, want %v", tt.a, tt.b, got, tt.want)
			}
		})
	}
}
...
func TestBitwiseOr(t *testing.T) {
	got, err := BitwiseOr(int64(0b1010), int64(0b0101))
	if err != nil || got != 0b1111 {
		t.Fatalf("BitwiseOr = %v, %v", got, err)
	}
	_, err = BitwiseOr("x", int64(1))
	if err == nil {
		t.Fatal("expected error for invalid a")
	}
	_, err = BitwiseOr(int64(1), "x")
	if err == nil {
...
func TestBitwiseXor(t *testing.T) {
	got, err := BitwiseXor(int64(0b1111), int64(0b0101))
	if err != nil || got != 0b1010 {
		t.Fatalf("BitwiseXor = %v, %v", got, err)
	}
	_, err = BitwiseXor("x", int64(1))
	if err == nil {
		t.Fatal("expected error for invalid a")
	}
	_, err = BitwiseXor(int64(1), "x")
	if err == nil {
		t.Fatal("expected error for invalid b")
	}
}
...
func TestBitwiseAnd(t *testing.T) {
	got, err := BitwiseAnd(int64(0b1110), int64(0b1011))
	if err != nil || got != 0b1010 {
		t.Fatalf("BitwiseAnd = %v, %v", got, err)
	}
	_, err = BitwiseAnd("x", int64(1))
	if err == nil {
		t.Fatal("expected error for invalid a")
	}
	_, err = BitwiseAnd(int64(1), "x")
	if err == nil {
		t.Fatal("expected error for invalid b")
	}
}
...
func TestBitwiseNot(t *testing.T) {
	got, err := BitwiseNot(int64(0))
	if err != nil || got != -1 {
		t.Fatalf("BitwiseNot = %v, %v", got, err)
	}
	_, err = BitwiseNot("x")
	if err == nil {
...
func TestShiftLeft(t *testing.T) {
	got, err := ShiftLeft(int64(1), int64(3))
	if err != nil || got != 8 {
		t.Fatalf("ShiftLeft = %v, %v", got, err)
	}
	_, err = ShiftLeft("x", int64(1))
	if err == nil {
		t.Fatal("expected error for invalid a")
	}
	_, err = ShiftLeft(int64(1), "x")
	if err == nil {
		t.Fatal("expected error for invalid b")
	}
}
...
func TestShiftRight(t *testing.T) {
	got, err := ShiftRight(int64(16), int64(2))
	if err != nil || got != 4 {
		t.Fatalf("ShiftRight = %v, %v", got, err)
	}
	_, err = ShiftRight("x", int64(1))
	if err == nil {
		t.Fatal("expected error for invalid a")
	}
	_, err = ShiftRight(int64(1), "x")
	if err == nil {
		t.Fatal("expected error for invalid b")
	}
}
...
func TestShiftRightUnsigned(t *testing.T) {
	got, err := ShiftRightUnsigned(int64(-1), int64(1))
	if err != nil || got != math.MaxInt64 {
		t.Fatalf("ShiftRightUnsigned = %v, %v", got, err)
	}
	_, err = ShiftRightUnsigned("x", int64(1))
	if err == nil {
		t.Fatal("expected error for invalid a")
	}
	_, err = ShiftRightUnsigned(int64(1), "x")
	if err == nil {
		t.Fatal("expected error for invalid b")
	}
}
...
func TestToInt64Err(t *testing.T) {
	tests := []struct {
		...
		{"int", int(3), 3, false},
		{"int8", int8(4), 4, false},
		{"int16", int16(5), 5, false},
		{"int32", int32(6), 6, false},
		{"int64", int64(7), 7, false},
		{"uint", uint(8), 8, false},
		{"uint8", uint8(9), 9, false},
		{"uint16", uint16(10), 10, false},
		{"uint32", uint32(11), 11, false},
		{"uint64", uint64(12), 12, false},
		{"float32 whole", float32(4), 4, false},
		{"float32 fractional", float32(1.5), 0, true},
		{"float32 NaN", float32(math.NaN()), 0, true},
		{"float64 whole", float64(5), 5, false},
		{"float64 fractional", float64(1.5), 0, true},
		{"float64 NaN", math.NaN(), 0, false},
		{"bool true", true, 1, false},
		{"bool false", false, 0, false},
		{"string int", "42", 42, false},
		{"string float", "3.0", 3, false},
		{"string dot float", ".5", 0, false},
		{"string invalid", "abc", 0, true},
		{"unknown type", struct{}{}, 0, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := toInt64Err(tt.in)
			if (err != nil) != tt.wantErr {
				t.Fatalf("toInt64Err(%v) err=%v, wantErr=%v", tt.in, err, tt.wantErr)
			}
			if !tt.wantErr && got != tt.want {
				t.Fatalf("toInt64Err(%v) = %d, want %d", tt.in, got, tt.want)
			}
		})
	}
}
