...
// fieldName resolves the expression name for a struct field.
// The `expr:"-"` tag hides the field; `expr:"name"` overrides it.
...
// structData lazily indexes a struct type's fields, including
// those promoted from anonymous (embedded) fields.
...
// finished reports whether all own and embedded fields
// have been visited — no more lazy work to do.
...
// structField returns the named field, populating the lazy cache
// as it walks own fields first, then embedded (anonymous) ones.
// parentEmbed, when non-nil, receives promoted fields as they're found.
...
// trySet adds sf to s.fields only if name is not already
// present — first definition wins (Go promotion rules).
// Also propagates upward to curParent if set.
...
// StructFields returns all exported fields of t (including
// promoted ones) as a name→Nature map. Deferences pointers first.
...
// methodset lazily indexes the exported methods of a type.
...
// method returns the named exported method, walking the set
// lazily and caching each one as it goes.
